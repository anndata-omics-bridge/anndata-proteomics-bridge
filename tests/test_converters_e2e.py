...
def _level_available_for(locator: RuleLocator, data_file: Path) -> bool:
    ...
    return document.level_is_available_for(locator.level, parse_params(param_paths[0], slug))
...
def test_end_to_end_conversion(locator: RuleLocator) -> None:
    ...
    data_file = find_test_data_for_version(rule.software_name, rule.software_version)
    ...
    adata = convert_to_anndata(df, rule)
