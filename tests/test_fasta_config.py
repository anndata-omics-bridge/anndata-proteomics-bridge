...
from anndata_proteomics.fasta.anndata_io import (
    read_fasta_config,
    write_fasta_config,
)
from anndata_proteomics.fasta.config import FastaConfig, resolve_fasta_config
...
def test_explicit_patterns_override_candidates() -> None:
    resolved = resolve_fasta_config(
        ("REV_P1", "CUSTOM_P2"),
        FastaConfig(decoy_patterns=(r"^CUSTOM_",)),
    )
...
def test_explicit_empty_tuple_disables_classification() -> None:
    resolved = resolve_fasta_config(
        ("REV_P1", "zz|C1|CONT"),
        FastaConfig(decoy_patterns=(), contaminant_patterns=()),
    )
    assert resolved.decoy.source == "explicit"
