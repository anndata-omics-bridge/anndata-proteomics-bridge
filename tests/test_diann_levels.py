...
def test_var_counts_follow_report_backed_levels(diann_full_subset: ConversionFixture) -> None:
...
def test_ion_var_preserves_peptide_peptidoform_and_protein_metadata(
    diann_full_subset: ConversionFixture,
) -> None:
...
def test_fragment_keys_are_positional(diann_full_subset: ConversionFixture) -> None:
...
def test_fragment_preserves_peptide_hierarchy(diann_full_subset: ConversionFixture) -> None:
    adata = _convert(diann_full_subset, "fragment")
    assert {
        "ProForma_peptide",
        "ProForma_peptidoform",
...
def test_protein_quant_is_kept_not_summed(diann_full_subset: ConversionFixture) -> None:
    ...
    value = float(np.asarray(adata[:, multi_pg].layers["PG_MaxLFQ"])[0, 0])
